...
if(isset($_GET['id'])){
	...
	$result=$connect->query("SELECT * FROM tb_konsumen WHERE id_kon='$id'") or die(mysql_error());
	...
	if(mysqli_num_rows($result) == 0){
		...
		$del = $connect->query("DELETE FROM tb_konsumen WHERE id_kon='$id'");
		...
		if($del){
			...
			echo '<a href="input_konsumen.php">Kembali</a>';	//membuat Link untuk kembali ke halaman beranda
