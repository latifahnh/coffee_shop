...
include '../koneksi.php';
//tampilan tabel
$result=$connect->query('SELECT * FROM tb_karyawan');
	    ...
	    <header>
	        <div class="container text-center" style="padding-bottom: 20px">
	            <div class="intro-text">
	                <div class="intro-heading" style="color: white">Daftar Karyawan</div>
	                <form action="tambah_karyawan.php"><button type="submit" method="post" class=" btn btn-xl" style="color: white"> Tambah Karyawan
	                </form>
	            </div>
	        </div>
	    </header>
				...
				<table class="table table-striped table-bordered jumbotron"style="background-color: rgba(236, 240, 241,0.8);color: #2C0A05;" align="right">
				   ...
				   <th class="text-center">Aksi</th>
					...
					while ($row=$result->fetch_assoc())
					{
						...
						echo "<td align='center'>".$row['id_kar']."</td>";
						echo "<td>".$row['kode_kar']."</td>";
						echo "<td>".$row['nama_kar']."</td>";
						echo "<td>".$row['bagian']."</td>";
						echo "<td>".$row['alm_kar']."</td>";
						echo "<td>".$row['tlp_kar']."</td>";
						echo '<td align="center"><a href="hapus.php?id='.$row['id_kar'].'" onclick="return confirm(\'Anda yakin untuk menghapus?\')"><button class="btn btn-danger btn-danger-outline">Hapus</button></a></td>';						   
						$no++;
					}
					?>
				</table>
